                    ...
                    <div className="folderModal" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div style={{display:"flex", justifyContent:"space-around", width:"70%", margin:"0 auto", marginTop:"6px"}}>
                            ...
                            <button onClick={async () => 
                                ...
                                fetchData(); 
                                setEditFolderModal(false);
                                const {data} =  await supabase.from("Assignments").select("*").eq("folder_id", selectedFolder.id);
                                data.map(child => (
                                    removeFromDatabase("Assignments", child.id)
                                ))
                                }}> 
                                Delete Folder 
                            </button>
            ...
            {editAssignmentModal && (
                <div className="overlay" onClick={() => setEditAssignmentModal(false)}>
                    ...
                    <div style={{display:"flex", justifyContent:"space-around", width:"90%", margin:"0 auto", marginTop:"6px"}}>
                        ...
                        <button onClick={() => {navigate(`../assignments/${selectedAssignment.id}`); setEditAssignmentModal(false)}}>
                            Go to Detailed View
                        </button>
                        <button onClick={async () => {await removeFromDatabase("Assignments", selectedAssignment.id); fetchData(); setEditAssignmentModal(false);}}>
                            Delete Assignment
                        </button>
                    </div>
